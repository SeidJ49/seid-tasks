...
def global_translation(gt_boxes, points, translate_std, noise_translate=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, ...]
        points: (M, 3 + C)
        translate_std: [std_x, std_y, std_z]
    Returns:
    """
    translate_std = np.asarray(translate_std, dtype=np.float32)
    if np.all(translate_std < 1e-6):
        return gt_boxes, points

    if noise_translate is None:
