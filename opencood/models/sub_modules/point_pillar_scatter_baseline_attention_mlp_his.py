...
class PointPillarScatterBaselineAttentionMlpHis(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        # ----------------------------------------- VERSION X ----------------------------------------------------------
        velocity_confidence = batch_dict['velocity_confidence']
        batch_confidence_masks = []
        for batch_idx in range(batch_size):
            confidence_feature = torch.zeros(1, self.nz * self.nx * self.ny, dtype=velocity_confidence.dtype, device=velocity_confidence.device)
