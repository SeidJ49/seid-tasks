...
class PillarVFEBaselineAttention(nn.Module):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range):
        ...
        # ----------------------------------------- VERSION X ----------------------------------------------------------
        self.use_velocity_branch = self.model_cfg.get('use_velocity_branch',
                                                      True)  # (x,y,z, v_rel_norm, v_r_norm, v_r_x_norm, v_r_y_norm)
    ...
    def forward(self, batch_dict):
        ...
        # ----------------------------------------- VERSION X ----------------------------------------------------------
        v_r = voxel_features[:, :, 3]
        sel = torch.abs(v_r) if self.vm_abs else v_r
