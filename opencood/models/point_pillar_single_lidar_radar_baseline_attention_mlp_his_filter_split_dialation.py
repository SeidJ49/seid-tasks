...
class PointPillarSingleLidarRadarBaselineAttentionMlpHisFilterSplitDialation(nn.Module):
    def __init__(self, args):
        super(PointPillarSingleLidarRadarBaselineAttentionMlpHisFilterSplitDialation, self).__init__()
    ...
    def forward(self, data_dict):
        ...
        B, C, H, W = original_dynamic_mask.shape
        weight = torch.ones(C, 1, k, k, device=original_dynamic_mask.device)

        neighbor_count = F.conv2d((original_dynamic_mask > 0).float(), weight, padding=pad, groups=C)
        valid_mask = (neighbor_count >= min_neighbors).float()
        clean_mask = original_dynamic_mask * valid_mask

        thr = 0.5
        mask_1 = clean_mask * (clean_mask > thr).float()
        mask_2 = clean_mask * (clean_mask <= thr).float()

        kernel_size = 7
        padding = kernel_size // 2

        dilated_mask_1 = F.max_pool2d(mask_1, kernel_size, stride=1, padding=padding)

        dilated = torch.maximum(dilated_mask_1, mask_2).clamp_(0.0, 1.0)
