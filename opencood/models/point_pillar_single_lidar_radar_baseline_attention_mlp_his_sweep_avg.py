...
class PointPillarSingleLidarRadarBaselineAttentionMlpHis(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        avged = F.avg_pool2d(m, kernel_size, stride=1, padding=padding)
        dilated = 0.5 * (maxed + avged)  # Hybrid statt hartem max

        sf = batch_dict['spatial_features']
        batch_dict['spatial_features'] = sf * (1.0 + self.gamma_pre * dilated)

        dyn_mask = F.interpolate(dilated.float(), size=spatial_features_2d.shape[-2:], mode='bilinear', align_corners=False).clamp_(0.0,
                                                                                                                                    1.0)

        assert dyn_mask.shape[0] == spatial_features_2d.shape[
            0], f"Batch size mismatch: dyn_mask {dyn_mask.shape[0]} vs spatial_features_2d {spatial_features_2d.shape[0]}"
