...
from opencood.models.common_modules.pillar_vfe_velocity import PillarVFE
from opencood.models.common_modules.point_pillar_scatter_velocity import PointPillarScatter
...
class MultiModalFusion(nn.Module):
    ...
    def forward(self, img_voxel, pc_dict):
        ...
        # Overlap Region: Where both pc_mask and img_mask are 1.
            ...
            # Fuses the concatenated features using self.multifuse.
            # This fused result is multiplied by pc_mask * img_mask to isolate it to the overlap region.
        # PC-Only Region: Where pc_mask is 1 and img_mask is 0.
            # Simply takes the original pc_voxel features, multiplied by pc_mask * (1 - img_mask).
        # Image-Only Region: Where img_mask is 1 and pc_mask is 0.
            # Calls self.img_fusion to get the attention-based binary mask (img_attn_mask). Remember img_attn_mask[0] is forced to 1s.
...
class PointPillarBM2CPRadarCamera(nn.Module):
    def create_frustum(self):
        # make grid in image plane
        ogfH, ogfW = self.data_aug_conf['final_dim']
        fH, fW = ogfH // self.downsample, ogfW // self.downsample
        ds = torch.tensor(depth_discretization(*self.grid_conf['ddiscr'], self.grid_conf['mode']), dtype=torch.float).view(-1, 1, 1).expand(
            -1, fH, fW)
    ...
    def __init__(self, args):
        super(PointPillarBM2CPRadarCamera, self).__init__()
        # cuda选择
        ...
        logger.info(f"total depth levels: {self.D}")
        self.camencode = ImgCamEncode(self.D, self.bevC, self.downsample, self.grid_conf['ddiscr'], self.grid_conf['mode'],
                                      img_args['use_depth_gt'], img_args['depth_supervision'])
        logger.info(f"Number of parameter CamEncode: {sum([param.nelement() for param in self.camencode.parameters()])}")

        # lidar branch
        pc_args = args['pc_params']
        self.pillar_vfe = PillarVFE(pc_args['pillar_vfe'], num_point_features=7, voxel_size=pc_args['voxel_size'], point_cloud_range=pc_args['lidar_range'])
        logger.info(f"Number of parameter pillar_vfe: {sum([param.nelement() for param in self.pillar_vfe.parameters()])}")
        self.scatter = PointPillarScatter(pc_args['point_pillar_scatter'])
        logger.info(f"Number of parameter scatter: {sum([param.nelement() for param in self.scatter.parameters()])}")
        ...
        logger.info(f"Number of parameter modal fusion: {sum([param.nelement() for param in self.fusion.parameters()])}")
        self.backbone = ResNetBEVBackbone(modality_args['bev_backbone'], input_channels=pc_args['point_pillar_scatter']['num_features'])
        logger.info(f"Number of parameter bevbackbone: {sum([param.nelement() for param in self.backbone.parameters()])}")
        ...
        if 'shrink_header' in modality_args:
            ...
            logger.info(f"Number of parameter shrink_conv: {sum([param.nelement() for param in self.shrink_conv.parameters()])}")
        ...
        #map_channels = input_dim // compress_raito if self.compression else modality_args['shrink_header']['dim'][
        #    0] if self.shrink_flag else sum(modality_args['bev_backbone']['num_upsample_filter'])
        ...
        logger.info(f"Number of parameter fusion_net: {sum([param.nelement() for param in self.fusion_net.parameters()])}")
    ...
    def get_geometry(self, image_inputs_dict):
        ...
        """
        # process image to get bev
        rots, trans, intrins, post_rots, post_trans = image_inputs_dict['rots'], image_inputs_dict['trans'], image_inputs_dict['intrins'], \
