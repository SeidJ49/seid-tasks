...
class OptimWrapper:
    ...
    def step(self):
        if self.true_wd:
            for lr, wd, group_non_bn, group_bn in zip(
...
class OneCycle:
    def __init__(self, optimizer, total_step, lr_max, moms, div_factor, pct_start, last_step=0):
        ...
        self.total_step = max(int(total_step), 1)
        ...
        for index, (start, func) in enumerate(lr_phases):
            ...
            self.lr_phases.append((int(start * self.total_step), int(end * self.total_step), func))
        for index, (start, func) in enumerate(mom_phases):
            end = mom_phases[index + 1][0] if index < len(mom_phases) - 1 else 1.0
            self.mom_phases.append((int(start * self.total_step), int(end * self.total_step), func))

        self.step_per_batch = True

    def step(self, *args, **kwargs):
        step = min(self.step_num, self.total_step)
