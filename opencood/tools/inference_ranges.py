...
def main():
    ...
    if 'opv2v' in opt.model_dir:
        from opencood.utils import eval_utils_ranges as eval_utils
        left_hand = True

    elif 'adver_city' in opt.model_dir:
        from opencood.utils import eval_utils_ranges as eval_utils  # Use the same eval_utils as opv2v
    ...
    result_stat = {0.3: {'tp': [], 'fp': [], 'gt': 0},
                   0.5: {'tp': [], 'fp': [], 'gt': 0, },
                   0.7: {'tp': [], 'fp': [], 'gt': 0, }}
    result_stat_short = {0.3: {'tp': [], 'fp': [], 'gt': 0},
                         0.5: {'tp': [], 'fp': [], 'gt': 0, },
                         0.7: {'tp': [], 'fp': [], 'gt': 0, }}
    result_stat_middle = {0.3: {'tp': [], 'fp': [], 'gt': 0},
                          0.5: {'tp': [], 'fp': [], 'gt': 0, },
                          0.7: {'tp': [], 'fp': [], 'gt': 0, }}
    result_stat_long = {0.3: {'tp': [], 'fp': [], 'gt': 0},
                        0.5: {'tp': [], 'fp': [], 'gt': 0, },
                        0.7: {'tp': [], 'fp': [], 'gt': 0, }}
    ...
    for i, batch_data in tqdm(enumerate(data_loader)):
        with torch.no_grad():
            ...
            if opt.fusion_method == 'late':
                ...
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_early_fusion(batch_data, model,
                                                                                                    opencood_dataset)
            elif opt.fusion_method == 'intermediate':
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_intermediate_fusion(batch_data,
                                                                                                           model,
                                                                                                           opencood_dataset)
            elif opt.fusion_method == 'no':
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_no_fusion(batch_data, model,
                                                                                                 opencood_dataset)
            elif opt.fusion_method == 'intermediate_with_comm':
                pred_box_tensor, pred_score, gt_box_tensor, comm_rates, mask, each_mask = inference_utils.inference_intermediate_fusion_withcomm(
                    batch_data, model, opencood_dataset)
            ...
            if opt.save_npy:
                ...
                inference_utils.save_prediction_gt(pred_box_tensor, gt_box_tensor, batch_data['ego']['origin_radar'][0],
                                                   i, npy_save_path)
            ...
            if opt.save_vis:
                ...
                simple_vis.visualize(None, gt_box_tensor, batch_data['ego']['origin_radar'][0], hypes['postprocess']['gt_range'],
                ...
                simple_vis.visualize(pred_box_tensor, gt_box_tensor, batch_data['ego']['origin_radar'][0],
    ...
    # Short range APs
    ap_30_short, ap_50_short, ap_70_short = eval_utils.eval_final_results(result_stat_short, opt.model_dir,
                                                                          range="short")
    # Middle range APs
    ap_30_middle, ap_50_middle, ap_70_middle = eval_utils.eval_final_results(result_stat_middle, opt.model_dir,
                                                                             range="middle")
    # Long range APs
    ap_30_long, ap_50_long, ap_70_long = eval_utils.eval_final_results(result_stat_long, opt.model_dir, range="long")
    ...
    with open(os.path.join(saved_path, 'result.txt'), 'a+') as f:
        msg = (
            f'Epoch: {epoch_id} | AP @0.3: {ap_30:.04f} | AP @0.5: {ap_50:.04f} | AP @0.7: {ap_70:.04f} | comm_rate: {comm_rates:.06f}\n'
            f'AP @0.5 short: {ap_50_short:.04f} | AP @0.7 short: {ap_70_short:.04f}\n'
            f'AP @0.5 middle: {ap_50_middle:.04f} | AP @0.7 middle: {ap_70_middle:.04f}\n'
            f'AP @0.5 long: {ap_50_long:.04f} | AP @0.7 long: {ap_70_long:.04f}\n'
        )
        if opt.comm_thre is not None:
            msg = (
                f'Epoch: {epoch_id} | AP @0.3: {ap_30:.04f} | AP @0.5: {ap_50:.04f} | AP @0.7: {ap_70:.04f} | comm_rate: {comm_rates:.06f} | comm_thre: {opt.comm_thre:.04f}\n'
                f'AP @0.5 short: {ap_50_short:.04f} | AP @0.7 short: {ap_70_short:.04f}\n'
                f'AP @0.5 middle: {ap_50_middle:.04f} | AP @0.7 middle: {ap_70_middle:.04f}\n'
                f'AP @0.5 long: {ap_50_long:.04f} | AP @0.7 long: {ap_70_long:.04f}\n'
            )
        if frame_times:
            msg += f'Average inference time per frame: {avg_time:.4f} seconds over {len(frame_times)} frames.\n'
        f.write(msg)
        print(msg)
